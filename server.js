...
const { webserver } = JSON.parse(fs_1.readFileSync(path_1.resolve("./package.json"), "utf8"));
async function server(app) {
    const middlewares = await async_require_context_1.default("./lib/src/middleware").catch(() => []);
    ...
    const runtimes = await async_require_context_1.default("./lib/src/runtime").catch(() => []);
    ...
    const endpoints = await async_require_context_1.default("./lib/api").catch(() => []);
    ...
    if (webserver.https.enabled) {
        let files = await promises_1.readdir(path_1.resolve(webserver.https.certs));
        files = files.map(file => path_1.resolve(webserver.https.certs, file));
        const key = files.filter(file => file.includes("key"))[0];
        const cert = files.filter(file => file.includes("cert"))[0];
        https_1.default.createServer({
            key: await promises_1.readFile(key, "utf8"),
            cert: await promises_1.readFile(cert, "utf8")
        }, app).listen(SSL_PORT);
